        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {ACHIEVEMENTS.map(a => {
              ...
              return (
                <div key={a.id} className={`border rounded-2xl p-4 text-center transition-all ${earned ? 'border-yellow-500/40 bg-yellow-500/10' : 'border-white/10 bg-white/5 opacity-50'}`}>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {courses.slice(0, 8).map(c => {
              ...
              return (
                <Link key={c.id} href={`/courses/${c.id}`} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-indigo-500/40 transition-all">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <div className="text-sm font-semibold text-white">{c.title}</div>
                      <div className="text-xs text-slate-400">{done}/{total} lessons</div>
                    </div>
